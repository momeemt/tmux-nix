      ...
      paneNavigationBinds = lib.concatStringsSep "\n" (
        lib.filter (x: x != "") (
          lib.mapAttrsToList (
            ...
            in
              if binding.key != null
              then mkBind binding.key binding.repeatable "select-pane -${direction}"
              else ""
          )
          keymaps.pane
        )
      ...
      paneResizingBinds = lib.concatStringsSep "\n" (
        lib.filter (x: x != "") (
          lib.mapAttrsToList (
            ...
            in
              if binding.key != null
              then mkBind binding.key binding.repeatable "resize-pane -${direction} ${toString binding.amount}"
              else ""
          )
          keymaps.resize
        )
